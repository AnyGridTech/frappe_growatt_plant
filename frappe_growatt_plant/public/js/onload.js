    ...
    const FetchSerialNumberPlant = async (values) => {
        ...
        const getPlantData = async (device) => {
            return await frappe
                ...
                .catch((e) => {
                const msg = e?.message || e;
                frappe.throw(__(`Error fetching plant data for device "${device.serialNumber}": ${msg}`));
                return {};
            });
        };
        ...
        const GetItemCodeUsingSerialNo = async (device) => {
            return await frappe.db
                .get_value("Serial No", { serial_no: device.serialNumber }, [
                "item_code",
            ])
                .then((r) => r.message?.item_code || "")
                .catch((e) => {
                const msg = e?.message || e;
                console.error(`Error fetching item code for serial ${device.serialNumber}:`, msg);
                return "";
            });
        };
        const GetItemCode = async (device) => {
            ...
            return await frappe.db
                ...
                .catch((e) => {
                const msg = e?.message || e;
                frappe.throw(__(`Error fetching item code using device model as reference: ${msg}`));
                return [];
            });
        };
        const CreateSerialNo = async (device) => {
            ...
            if (!item) {
                frappe.msgprint(`Failed to create Serial No for "${device.serialNumber}".`);
                return;
            }
            frappe.dom.freeze(__("Processing devices..."));
            return item;
        ...
        deviceLoop: for (const device of devices) {
            ...
            if (itemcode_first_try.length === 0) {
                ...
                if (!serial_no)
                    return frappe.throw(`Failed to create Serial No "${device.serialNumber} with model "${device.deviceModel}".`);
            ...
            plantLoop: for (const plant of other_plants) {
                if (plant.equipamentos_ativos_na_planta.length === 0) {
                    return frappe.throw(`Strange behaviour: Plant "${plant.name}" has no active equipment BUT it was fetched as having the serial number "${device.serialNumber}".`);
                }
                const history_serials = plant.historico_de_equipamentos || [];
                const existing_serials = plant.equipamentos_ativos_na_planta.filter((eq) => eq.serial_number !== device.serialNumber);
